...
#'
#' @param data A data frame containing data from a partially nested, two-facet design with the two facets crossed and then nested within subjects. Must have one or more columns for metrics of interest, one column for labeling subjects, and two columns for labeling facets.
...
#'}))
dstudy_p_nested4 <- function(data, col.scores, col.subjects, col.facet1, col.facet2, seq1, seq2, threshold = 0.7,
  ...
  formula1 <- glue::glue("{col.scores} ~ (1|{col.subjects}/{col.facet1}) + (1|{col.subjects}:{col.facet2})")
  ...
  row2 <- paste0("var_", col.subjects, ":", col.facet1)
  ...
  for (i in seq(1, nrow(final_df))) {
    ...
    var_df <- var_df %>%
      dplyr::mutate(
        ...
        G_coef = new_Person/(new_Person + new_Person_Item +
