#' Execute a Two-Faceted, Fully-Nested Bayesian D-Study - i:o:p
#'
#' @param data A data frame containing data from a random, fully nested two-facet design with one facet nested within another facet then nested within subjects. Must have one or more columns for metrics of interest, one column for labeling subjects, and two columns for labeling facets.
...
#' @param col.facet1 The name of the column containing the labels for the innermost nested facet. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
#' @param col.facet2 The name of the column containing the labels for the outer nested facet. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
#' @param seq1 A sequence of integers defining the interval at which to test the innermost nested facet. Enter a vector, or use the seq() function directly.
#' @param seq2 A sequence of integers defining the interval at which to test the outer nested facet. Enter a vector, or use the seq() function directly.
...
#' @param prior An optional set of prior distributions for the variance components, specified by the user through the set_prior() function in brms. To ensure correctly formatted priors, the user should first use the get_prior() function with the formula "col.scores ~ (1|col.subjects/col.facet2)". Type ?brms::set_prior in the console for more information. NULL by default.
...
#'Score <- c(6,6,7,6,5,5,1,3,1,2,2,2,5,5,4,5,4,5,10,9,8,10,
#'           10,10,5,6,6,6,5,6)
#'sample_data <- data.frame(Person, Item, Occasion, Score)
#'dstudy_nested2(data = sample_data, col.scores = "Score",
#'                col.subjects = "Person", col.facet1 = "Item",
#'                col.facet2 = "Occasion", seq1 = seq(1,5,1),
#'                seq2 = seq(1,3,1), threshold = 0.7,
#'                warmup = 1000, iter = 4000, chains = 1)
#'}))
dstudy_nested2 <- function(data, col.scores, col.subjects, col.facet1, col.facet2, seq1, seq2, threshold = 0.7,
  ...
  formula1 <- glue::glue("{col.scores} ~ (1|{col.subjects}/{col.facet2})")
  ...
  row2 <- paste0("var_", col.subjects, ":", col.facet2)
  row3 <- "var_Error"
  ...
  for (i in seq(1, nrow(final_df))) {
    ...
    var_df <- var_df %>%
      dplyr::mutate(
        ...
        G_coef = new_Person/(new_Person + new_Person_Occasion + new_Error))
