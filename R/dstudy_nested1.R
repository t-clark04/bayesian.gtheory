#' Execute a One-Faceted Nested Bayesian D-Study -- i:p
#'
#' @param data A data frame containing data from a random, fully nested one-facet design with the facet nested within subjects. Must have one or more columns for metrics of interest, one column for labeling subjects, and one column for labeling the facet.
...
#' @param col.facet The name of the column containing the labels for the nested facet. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
#' @param seq A sequence of integers defining the interval at which to test the nested facet. Enter a vector, or use the seq() function directly.
...
#' @param prior An optional set of prior distributions for the variance components, specified by the user through the set_prior() function in brms. To ensure correctly formatted priors, the user should first use the get_prior() function with the formula "col.scores ~ (1|col.subjects)". Type ?brms::set_prior in the console for more information. NULL by default.
...
#'dstudy_nested1(data = sample_data, col.scores = "Score",
#'                col.subjects = "Person", col.facet = "Item",
#'                seq = seq(1,5,1), threshold = 0.7, warmup = 1000,
#'                iter = 4000, chains = 1)
#'}))
dstudy_nested1 <- function(data, col.scores, col.subjects, col.facet, seq, threshold = 0.7,
  ...
  formula1 <- glue::glue("{col.scores} ~ (1|{col.subjects})")
  ...
  suppressWarnings(var_df <- samples[2:3])
  ...
  row2 <- "var_Error"
  rownames(variance_comps) <- c(row1, row2)
  ...
  for (i in seq(1, nrow(final_df))) {
    ...
    var_df <- var_df %>%
      dplyr::mutate(
        ...
        G_coef = new_Person/(new_Person + new_Error)
      )
