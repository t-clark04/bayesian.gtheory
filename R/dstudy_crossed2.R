...
#'dstudy_crossed2(data = sample_data, col.scores = "Score",
#'                col.subjects = "Person", col.facet1 = "Item",
#'                col.facet2 = "Occasion", seq1 = seq(1,5,1),
#'                seq2 = seq(1,3,1), threshold = 0.7, facet.fixed = NULL,
...
dstudy_crossed2 <- function(data, col.scores, col.subjects, col.facet1, col.facet2, seq1, seq2, threshold = 0.7, facet.fixed = NULL,
  ...
  row2 <- paste0("var_", col.facet1)
  row3 <- paste0("var_", col.facet2)
  row4 <- paste0("var_", col.subjects, ":", col.facet1)
  row5 <- paste0("var_", col.subjects, ":", col.facet2)
  row6 <- paste0("var_", col.facet1, ":", col.facet2)
  row7 <- "var_Error"
  rownames(variance_comps) <- c(row1, row2, row3, row4, row5, row6, row7)
