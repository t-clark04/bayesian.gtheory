...
#'
#' @param data A data frame containing data from a partially nested, two-facet design with one facet nested within the crossing of the other facet with subjects. Must have one or more columns for metrics of interest, one column for labeling subjects, and two columns for labeling facets.
...
#' @param col.facet1 The name of the column containing the labels for the nested facet. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
#' @param col.facet2 The name of the column containing the labels for the facet crossed with subjects. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
...
#' @param prior An optional set of prior distributions for the variance components, specified by the user through the set_prior() function in brms. To ensure correctly formatted priors, the user should first use the get_prior() function with the formula "col.scores ~ (1|col.subjects) + (1|col.facet2) + (1|col.subjects:col.facet2)". Type ?brms::set_prior in the console for more information. NULL by default.
...
#'dstudy_p_nested3(data = sample_data, col.scores = "Score", col.subjects = "Person",
#'                 col.facet1 = "Item", col.facet2 = "Occasion", seq1 = seq(1,5,1),
#'                 seq2 = seq(1,3,1), threshold = 0.5, warmup = 1000, iter = 4000, chains = 1)
#'}))
dstudy_p_nested3 <- function(data, col.scores, col.subjects, col.facet1, col.facet2, seq1, seq2, threshold = 0.7,
  ...
  formula1 <- glue::glue("{col.scores} ~ (1|{col.subjects}) + (1|{col.facet2}) + (1|{col.subjects}:{col.facet2})")
  ...
  suppressWarnings(var_df <- samples[2:5])
  ...
  row4 <- "var_Error"
  rownames(variance_comps) <- c(row1, row2, row3, row4)
  ...
  for (i in seq(1, nrow(final_df))) {
    ...
    var_df <- var_df %>%
      dplyr::mutate(
        ...
        G_coef = new_Person/(new_Person + new_Occasion +
