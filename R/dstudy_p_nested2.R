#' Execute a Partially Nested Bayesian D-Study -- p x (i:o)
#'
#' @param data A data frame containing data from a partially nested, two-facet design with one facet nested within the other, then crossed with subjects. Must have one or more columns for metrics of interest, one column for labeling subjects, and two columns for labeling facets.
...
#' @param col.facet2 The name of the column containing the labels for the non-nested facet. Must follow C++ naming conventions (only letters, numbers, and underscores; no spaces or hyphens!). Enter as a string.
...
dstudy_p_nested2 <- function(data, col.scores, col.subjects, col.facet1, col.facet2, seq1, seq2, threshold = 0.7,
  ...
  var_df <- var_df %>%
    dplyr::mutate(
      ...
      var_Occasion_Item = .[[glue::glue("sd_{col.facet2}:{col.facet1}__Intercept")]]^2,
      var_Error = sigma^2
    ) %>%
    dplyr::select(
  ...
  row4 <- paste0("var_", col.facet1, ":", col.facet2)
  row5 <- "var_Error"
  ...
  for (i in seq(1, nrow(final_df))) {
    ...
    var_df <- var_df %>%
      dplyr::mutate(
        ...
        new_Occasion_Item = var_Occasion_Item/(n_i*n_o),
